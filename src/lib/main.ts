...
export const revealTile = (tileIndex: number, isStepless?: undefined | boolean) => {
  if (!isStepless) addStep();
  changeTileContent(tileIndex, steps.v[0].tileContents[tileIndex] as number, true);
};

export const colorTile = (tileIndex: number, color: string, isStepless?: undefined | boolean) => {
  if (!isStepless) addStep();
  steps.v[stepsIndexer.v].tileColors[tileIndex] = color;
  revealTile(tileIndex, true);
};

export const colorTiles = (startIndex: number, endIndex: number, color: string, isStepless?: undefined | boolean) => {
  if (!isStepless) addStep();
  for (let i = startIndex; i <= endIndex; i++) colorTile(i, color, true);
};

export const newEmptyTile = () => {
  steps.v[stepsIndexer.v].metaTileContents.push(null);
};

export const changeEmptyTileContent = (tileIndex: number, content: number | null, isStepless?: undefined | boolean) => {
  if (!isStepless) addStep();
...
export const finalResult = (target: number, targetIndex: number) => {
  addStep();
